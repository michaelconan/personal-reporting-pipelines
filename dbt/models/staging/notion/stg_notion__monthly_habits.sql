with monthly_habits as (

    select
        parent__data_source_id as database_id,
        id as page_id,
        cast(left({{ json_extract_value('properties__date__date', "'$.start'") }}, 10) as date)
            as page_date,
        {{ extract_notion_title('properties__name__title') }} as page_name,
        properties__budget__checkbox as did_budget,
        properties__serve__checkbox as did_serve,
        properties__travel__checkbox as did_travel,
        properties__blog__checkbox as did_blog,
        properties__goal_review__checkbox as did_goal_review,
        properties__training__checkbox as did_training,
        created_time as created_at,
        last_edited_time as updated_at
    from
        {{ source('notion', 'data_source_monthly_habits') }}

),

valid_monthly_habits as (
-- Drop placeholder/junk pages (no title or no date) so they never pollute the
-- habit grain downstream even if an import accidentally lacks the filter.
    select *
    from monthly_habits
    where page_date is not null
      and page_name is not null
      and page_name != ''
),

unique_monthly_habits as (

    {{ dbt_utils.deduplicate(
        relation='valid_monthly_habits',
        partition_by='page_id',
        order_by='updated_at desc'
      )
    }}

)

select * from unique_monthly_habits
